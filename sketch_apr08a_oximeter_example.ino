/*
 * 
 * modified example for serial output
...
//intializing the liquid display library and saying what pins are used to communicate
// (RS, EN, D4,D5,D6,D7)
...
//LiquidCrystal lcd(1,2,3,5,16, 2);
...
//   lcd.backlight();
  ...
  // for(int i=0;i<8;i++) lcd.createChar(i, sym[i]);
   ...
   lcd.begin(16,2); //state how large is the display (characters)
...
void loop ()
{
  ...
  for (int i = 0; i < measures; i++) { measuresPeriods[i]=0; measuresR[i]=0; } //for statement (initialization; condition; increment)
  ...
  int rise_count;
  int n;
  long int last_beat;//long variable are extended variables for number storage that store 32 bits 
...
//    Serial.print(",");
//    if (avBPM < 200 & avBPM > 20) {
//      Serial.print(avBPM);
//      avBPMprev=avBPM;
//    }
//    else {
//      Serial.print(avBPMprev);
//    }
//    Serial.print(",");
//    if (SpO2 < 200 & SpO2 > 1) {
//      Serial.println(SpO2);
